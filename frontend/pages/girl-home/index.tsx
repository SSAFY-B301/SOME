...
export default function Home() {
  ...
  const girlListDetailState = useSelector(
  ...
  return (
    ...
    >
      <div
        ...
        className="absolute z-10 flex items-center justify-center w-10 h-10 bg-white rounded-full shadow-md top-16 left-2"
      >
      ...
      >
        ...
        >
          <div className="relative w-52 h-52">
            <div
              className={`absolute w-52 h-52 rounded-full ${girlStyles.loader_back} ${girlStyles.loader_animation} ${girlStyles.loader}`}
            ></div>
            {userStatus === "success" && (
              <div
                className={`top-20 left-20 z-10 absolute w-12 h-12 bg-center bg-cover rounded-full ${girlStyles.inner}`}
                style={{ backgroundImage: `url(${getUserInfo.user_img})` }}
              ></div>
            )}
            {userStatus !== "success" && (
              <div
                className={`top-20 left-20 z-10 absolute w-12 h-12 bg-gray-400 rounded-full`}
              ></div>
            )}
          </div>
        </CustomOverlayMap>
              ...
              if (result.photoList.length !== 0) {
                return (
                  <CustomOverlayMap
                    key={index}
                    position={{
                      lat: result.markGps.latitude,
                      lng: result.markGps.longitude,
                    }}
                  >
                    <div
                      onClick={() => openDetailList(result.markGps)}
                      className="relative w-24 h-24 bg-white rounded-lg shadow-md"
                    >
                      <div
                        style={{
                          background:
                        ...
                        className="absolute flex items-center justify-center w-6 h-6 bg-red-500 rounded-full -right-2 -top-2"
                      >
                        <p className="text-xs text-white">
                          {result.totalPhotoCnt}
                        </p>
                      </div>
