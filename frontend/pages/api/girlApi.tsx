...
function getGirlList() {
  ...
  const { data: queryData, isLoading: isGirlListLoading } = useQuery(
    ["girlList", location],
    () =>
      customGirlAxios.get(
        `/album/list?latitude=${location.lat}&longitude=${location.lng}`
      )
  );
...
function getGirlListDetail() {
  ...
  const { data: queryData, isLoading: isGirlListLoading } = useQuery(
    [`girlListDetail`, girlListDetailState],
    () =>
      customGirlAxios.get(
        `/album/list/detail?latitude=${location.lat}&longitude=${location.lng}&section=${girlListDetailState.section}&sort=${girlListDetailState.order}&page=${girlListDetailState.page}&size=${girlListDetailState.size}`
      ),
    {}
  );
...
function useMutationGirl() {
  ...
  const { mutate: girlUploadMutation } = useMutation(
    (requestData: GirlRequestPartType) =>
      customGirlAxios.post(
        "/album/upload?latitude=" + location.lat + "&longitude=" + location.lng,
  ...
  const { mutate: girlLikeMutation } = useMutation(
    (requestData: GirlLikeRequestType) =>
      customGirlAxios.put("/photo/like", requestData),
    {
      onSuccess: () => {
        queryClient.invalidateQueries("girlList"); // queryKey 유효성 제거
        queryClient.invalidateQueries("girlListDetail"); // queryKey 유효성 제거
