...
const VoteModal = ({ clickVote, requestSns, photoId }: Props): JSX.Element => {
  ...
  return isSnsRequest ? (
    ...
    <div
      ...
      onClick={() => clickVote(photoId)}
    >
      <div className="w-11/12 h-24 opacity-90 flex flex-col justify-end items-center">
        <div className="w-full h-3/6 opacity-90 bg-white dark:bg-dark-block rounded-xl flex flex-col justify-center items-center">
          <div className="w-full h-4/6 border-b-2 flex justify-center items-center text-gray-400 dark:text-white text-sm">
            공유 요청 알림이 보내집니다.
          </div>
          <button
            className="w-full h-1/2 flex justify-center items-center text-lg font-bold text-blue-500"
            onClick={requestSns}
          >
            알림 보내기
          </button>
